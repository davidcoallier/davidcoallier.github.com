...
# @TODO This analysis is likely to miss a lot of data. What we should do
# is make a list of tags we want to retrieve from Stackoverflow, and find
# a way to find those through http://stackoverflow.com/filter/tags-for-index?filter={lang}&tab=Popular
# but SO seems to be restricting access to the referer SO. Will investigate further.
...
def searchFromArchive(url):
    search_url  = 'http://web.archive.org/web/20130101000000*/http://stackoverflow.com/tags'
    search      = requests.get(search_url)
    return search.content

content = searchFromArchive('http://web.archive.org/web/20130101000000*/http://stackoverflow.com/tags')
soup_search = BeautifulSoup(content)
